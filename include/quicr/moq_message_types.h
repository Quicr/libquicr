...
// TODO(Suhas): This needs to merge into one message types file
...
namespace quicr::messages {
  ...
  constexpr u_int8_t MESSAGE_TYPE_OBJECT_PREFER_DATAGRAM   = 0x1;
  ...
  enum struct ParameterType : uint8_t {
    Role = 0x0,
    Path = 0x1,
    AuthorizationInfo = 0x2, // version specific
  };

  struct MoqParameter {
    ParameterType key;
    quicr::bytes value; // Len and Value encoding
  };
  ...
  struct MoqSubscribe {
    ...
    Location end_group;
    Location end_object;
    TrackRequestParameters track_params;
 };
  ...
  MessageBuffer& operator<<(MessageBuffer &buffer, const TrackRequestParameters &msg);
  MessageBuffer& operator>>(MessageBuffer &buffer, TrackRequestParameters &msg);
