...
quicr::FullTrackName cpp_track_name_from_c(const qbridge_full_track_name_t* c_track_name) {
    if (!c_track_name ||
        c_track_name->track_name.tuple_len == 0 ||
 ...
 */
void c_track_name_from_cpp(qbridge_full_track_name_t* c_track_name, const quicr::FullTrackName& cpp_track_name) {
    if (!c_track_name) return;
...
quicr::ClientConfig cpp_client_config_from_c(const qbridge_client_config_t* c_config) {
    ...
    if (c_config) {
        config.connect_uri = std::string("moq://") + c_config->server_hostname + ":" + std::to_string(c_config->server_port);
...
protected:
    BridgeClient(const quicr::ClientConfig& config) : quicr::Client(config) {}
...
public:
    static std::shared_ptr<BridgePublishTrackHandler> Create(
...
struct qbridge_publish_track_handler {
    ...
    qbridge_publish_track_handler(const qbridge_publish_track_config_t* config,
        ...
        if (config) {
            ...
            cpp_handler = BridgePublishTrackHandler::Create(
                full_track_name,
                track_mode,
...
struct qbridge_subscribe_track_handler {
    ...
    qbridge_subscribe_track_handler(const qbridge_subscribe_track_config_t* config,
        ...
        if (config) {
            ...
            cpp_handler = BridgeSubscribeTrackHandler::Create(
                full_track_name,
...
public:
    static std::shared_ptr<BridgeFetchTrackHandler> Create(
        ...
        return std::shared_ptr<BridgeFetchTrackHandler>(new BridgeFetchTrackHandler(
            full_track_name, priority, group_order, start_group, end_group, start_object, end_object, callback, user_data));
    }
...
protected:
    BridgeFetchTrackHandler(const quicr::FullTrackName& full_track_name,
                           ...
                           qbridge_object_received_callback_t callback,
                           void* data)
        : quicr::FetchTrackHandler(full_track_name, priority, group_order, start_group, end_group, start_object, end_object)
        , received_callback(callback)
        , user_data(data) {}
...
struct qbridge_fetch_track_handler {
    ...
    qbridge_fetch_track_handler(const qbridge_fetch_track_config_t* config,
        ...
        if (config) {
            ...
            cpp_handler = BridgeFetchTrackHandler::Create(
                full_track_name,
                static_cast<quicr::messages::SubscriberPriority>(config->priority),
                static_cast<quicr::messages::GroupOrder>(config->group_order),
...
qbridge_result_t qbridge_client_subscribe_namespace(qbridge_client_t* client,
    ...
    const auto cpp_namespace = cpp_namespace_from_c(ns);
    client->cpp_client->SubscribeNamespace(cpp_namespace);
    return QBRIDGE_OK;
}
...
qbridge_result_t qbridge_client_unsubscribe_namespace(qbridge_client_t* client,
    ...
    const auto cpp_namespace = cpp_namespace_from_c(ns);
    client->cpp_client->UnsubscribeNamespace(cpp_namespace);
    return QBRIDGE_OK;
}
...
// Track operations
qbridge_result_t qbridge_client_publish_track(qbridge_client_t* client,
...
qbridge_result_t qbridge_client_unpublish_track(qbridge_client_t* client,
                                                 qbridge_publish_track_handler_t* handler) {
    if (!client || !client->cpp_client || !handler || !handler->cpp_handler) {
        return QBRIDGE_ERROR_INVALID_PARAM;
    }
...
qbridge_result_t qbridge_client_fetch_track(qbridge_client_t* client,
...
qbridge_result_t qbridge_publish_object(qbridge_publish_track_handler_t* handler,
                                         const qbridge_object_t* object) {
...
// Utility functions
qbridge_result_t qbridge_namespace_from_string(qbridge_namespace_t* ns,
...
const char* qbridge_result_to_string(qbridge_result_t result) {
    switch (result) {
        case QBRIDGE_OK: return "Success";
        case QBRIDGE_ERROR_INVALID_PARAM: return "Invalid parameter";
        case QBRIDGE_ERROR_NOT_CONNECTED: return "Not connected";
        case QBRIDGE_ERROR_NOT_AUTHORIZED: return "Not authorized";
        case QBRIDGE_ERROR_NO_MEMORY: return "Out of memory";
        case QBRIDGE_ERROR_TIMEOUT: return "Timeout";
        case QBRIDGE_ERROR_TRACK_NOT_FOUND: return "Track not found";
        case QBRIDGE_ERROR_ALREADY_EXISTS: return "Already exists";
        case QBRIDGE_ERROR_INTERNAL: return "Internal error";
        case QBRIDGE_ERROR_INVALID_STATE: return "Invalid state";
...
const char* qbridge_status_to_string(qbridge_connection_status_t status) {
    switch (status) {
        case QBRIDGE_STATUS_NOT_CONNECTED: return "Not connected";
        case QBRIDGE_STATUS_CONNECTING: return "Connecting";
        case QBRIDGE_STATUS_CONNECTED: return "Connected";
        case QBRIDGE_STATUS_DISCONNECTING: return "Disconnecting";
        case QBRIDGE_STATUS_READY: return "Ready";
        case QBRIDGE_STATUS_ERROR: return "Error";
