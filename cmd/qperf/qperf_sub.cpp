...
namespace {
    ...
    struct PerfSubscriberDelegate : public quicr::SubscriberDelegate
    {
        ...
        PerfSubscriberDelegate(std::shared_ptr<spdlog::logger> l)
          : logger{ std::move(l) }
        {
        }

        void onSubscribeResponse(const quicr::Namespace&, const quicr::SubscribeResult&)
        {
            ++sub_responses_received;
            cv.notify_one();
        }

        void onSubscriptionEnded(const quicr::Namespace&, const quicr::SubscribeResult::SubscribeStatus&)
        {
        ...
        void onSubscribedObjectFragment(const quicr::Name&, uint8_t, const uint64_t&, bool, quicr::bytes&&)
        {
...
main(int argc, char** argv)
{
    ...
    for (std::uint16_t i = 0; i < streams; ++i) {
        auto [entry, _] =
          delegates.emplace(quicr::Namespace{ ns.name() + ((0x0_name + i) << (128 - ns.length())), ns.length() },
                            std::make_shared<PerfSubscriberDelegate>(logger));
