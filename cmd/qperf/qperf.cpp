...
namespace {
    ...
    /**
     * PublishDelegate that handles publish intents and notifies waiting threads that the intent was received.
     */
    struct PerfPublishDelegate : public quicr::PublisherDelegate
    {
        void onPublishIntentResponse(const quicr::Namespace&, const quicr::PublishIntentResult&) override
        {
    ...
    inline void loop_for(const D& duration, const I& interval, const F& func, Args&&... args)
    {
    ...
    std::string format_bitrate(const std::uint32_t& bitrate)
    {
        if (bitrate > 1e9) {
...
main(int argc, char** argv)
{
    ...
    quicr::Client client(info, result["endpoint_id"].as<std::string>(), chunk_size, config, logger);
    ...
    try {
        ...
        SPDLOG_LOGGER_CRITICAL(
          logger, "Failed to connect to relay '{0}:{1}' eith exception: {2}", info.hostname, info.port, e.what());
        return EXIT_FAILURE;
    } catch (...) {
    ...
    for (std::uint16_t i = 0; i < streams; ++i) {
        auto& publish_ns = namespaces.emplace_back(ns.name() + ((0x0_name + i) << (128 - ns.length())), ns.length());
        ...
        client.publishIntent(std::make_shared<PerfPublishDelegate>(),
                             ...
                             "",
                             {},
                             reliable ? quicr::TransportMode::ReliablePerGroup : quicr::TransportMode::Unreliable);
    }
    ...
    for (quicr::Namespace& pub_ns : namespaces) {
        threads.emplace_back([&] {
            ...
            ::loop_for(duration, interval, [&] {
                ...
                quicr::Name name = pub_ns.name() | (~(~0x0_name << (128 - pub_ns.length())) &
    ...
    cv.wait(lock, [&] { return terminate.load() || finished_publishers == streams; });
