  ...
  , public SpdlogLogger
{
  ...
  public:
    ...
    try {
        json info = json::from_bson(data);

        logger_->log(spdlog::source_loc(info["location"]["file_name"].get_ref<const std::string&>().c_str(),
                                        info["location"]["line"].get<int>(),
                                        info["location"]["function_name"].get_ref<const std::string&>().c_str()),
                     ConvertLevelType(static_cast<Level>(info["level"].get<int>())),
                     "PUBLISHER LOG: " + info["msg"].get_ref<const std::string&>());
    } catch (const std::exception& e) {
