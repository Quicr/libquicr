...
namespace quicr::ctrl_messages {  
    ...
    // usings
    using SupportedVersion = std::vector<std::uint64_t>;
    using SetupParameters = std::vector<quicr::Parameter>;
    ...
    struct SubscribeOk
    {
        ...
        std::optional<SubscribeOk::Group_1> group_1;
        SubscribeParameters subscribe_parameters;
    };
...
Bytes& operator<<(Bytes& buffer, const std::vector<quicr::Parameter>& vec);

BytesSpan operator>>(BytesSpan buffer, std::vector<quicr::Parameter>& vec);
