...
class ProtocolMessageParser:
    ...
    def __init__(self):
        ...
        self.field_type_map = {
            ...
            "Subscribe Parameters": "quicr::Parameter",
            "Setup Parameters": "quicr::Parameter",
            "New Session URI": "quicr::Bytes",
            "Parameters": "quicr::Parameter",
    ...
    def parse_field_definition(
        ...
        # repeated ... (True)
        #    varible length (..)
        #        field_type = field_type_map(Foo Bars) = FooBars
        #        cpp_type = FooBar // using std::vector<FooBars> FooBars;
        #
        # Fox Trots(i) ...
        # repeated ... (True)
        #     variable length (False)
        #        field_type = field_type_mae(Fox Trots) = "FoxTrots", is_repeated
        #        cpp_type = field_type_map(i) = std::uint64_t // using std::vector<std::uint64_t> FoxTrot
        #        spec_type = i
        #
        # Whiskey Tango(i)
        # repeated (False)
        #    variable length (False)
        #        field_type = field_type_map(i)
        ...
        if is_variable_length:
            ...
            cpp_type = self.field_type_map.get(
                spec_type, '#error "field_type_map look error for {spec_type}"'
            )
        ...
        if is_repeated:  # this is a vector in C++
            ...
            # self.using_map[cpp_using_name] = cpp_using_type
