...
namespace quicr {
      ...
      std::shared_ptr<const std::vector<uint8_t>> data)
    {
...
#if 0
                    auto stream_id =
                      session->CreateStream(LockPublishDataContext(), GetDefaultPriority());
        ...
        auto result =
          session->Enqueue(GetPublishDataContext(), stream_id, data, default_priority_, default_ttl_, 0, eflags);
