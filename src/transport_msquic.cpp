...
namespace quicr {
    namespace {
        ...
        std::shared_ptr<MsQuicStream> AsMsQuicStream(const std::shared_ptr<Stream>& stream)
        {
            return std::dynamic_pointer_cast<MsQuicStream>(stream);
        }
    }
                                                   ...
                                                   QUIC_CONNECTION_EVENT& event)
    {
        switch (event.Type) {
            ...
            case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED: {
                ...
                std::uint32_t id_size = sizeof(stream_id);
                const auto status =
                  api_->GetParam(event.PEER_STREAM_STARTED.Stream, QUIC_PARAM_STREAM_ID, &id_size, &stream_id);
                if (QUIC_FAILED(status)) {
                    return status;
                }

                auto stream = std::make_shared<MsQuicStream>(stream_id, connection->GetID());
            ...
            case QUIC_CONNECTION_EVENT_DATAGRAM_RECEIVED: {
                ...
                QueueCallback([connection]() { connection->OnRecvDgram(); });
                break;
            }
    ...
    {
        try {
            auto stream = static_cast<MsQuicStream*>(context)->SharedFromThis();
    ...
    QUIC_STATUS MsQuicTransport::OnStreamEvent(const std::shared_ptr<MsQuicStream>& stream, QUIC_STREAM_EVENT& event)
    {
        switch (event.Type) {
            case QUIC_STREAM_EVENT_START_COMPLETE: {
                HQUIC failed_stream_handle = nullptr;
                {
                    ...
                    if (QUIC_SUCCEEDED(event.START_COMPLETE.Status)) {
                        stream->SetAssignedId(event.START_COMPLETE.ID);
                        stream->id_ready = true;
            ...
            case QUIC_STREAM_EVENT_SEND_COMPLETE: {
                ...
                {
                    std::lock_guard lock(send->stream->metrics_mutex);
                    send->stream->metrics.tx_stream_cb++;
                    if (send->buffer.Length > 0) {
                        send->stream->metrics.tx_object_duration_us.AddValue(
                          static_cast<std::uint64_t>(tick_service_->get().count()) - send->enqueued_at);
                    }
                }
                send->stream->outstanding_sends.fetch_sub(1);
                break;
            }
            ...
            case QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE: {
                ...
                QueueCallback([connection, stream_id = stream->GetStreamId()]() {
                    if (connection) {
    ...
    void MsQuicTransport::OnStreamReceive(const std::shared_ptr<MsQuicStream>& stream, const QUIC_STREAM_EVENT& event)
    {
        ...
        if (total_length > 0) {
            ...
            if (auto connection = stream->connection.lock()) {
                QueueCallback([connection, stream]() {
                    connection->OnRecvStream(stream->GetStreamId(), stream->rx_context, stream, stream->is_bidir);
                });
            }
        }
    ...
    void MsQuicTransport::NotifyStreamClosed(const std::shared_ptr<MsQuicStream>& stream, const StreamClosedFlag flag)
    {
        ...
        QueueCallback([connection, stream, flag]() {
            connection->OnStreamClosed(stream->GetStreamId(), stream->rx_context, flag);
        });
    }

    std::shared_ptr<MsQuicStream> MsQuicTransport::CreateStream(const std::shared_ptr<Connection>& connection,
                                                                const bool bidir,
                                                                const std::uint8_t priority)
    {
        ...
        auto stream = std::make_shared<MsQuicStream>(0, msquic_connection->GetID());
        stream->transport = this;
        stream->connection = msquic_connection;
        stream->is_bidir = bidir;
    ...
    {
        auto msquic_connection = AsMsQuicConnection(connection);
        auto msquic_stream = AsMsQuicStream(stream);
        if (!msquic_connection) {
            return TransportError::kInvalidConnContextId;
        }
        if (!msquic_stream || msquic_stream->GetConnectionID() != msquic_connection->GetID() ||
            !msquic_stream->IsOpen() || msquic_stream->handle == nullptr) {
        ...
        {
            std::lock_guard lock(msquic_stream->metrics_mutex);
            msquic_stream->metrics.enqueued_objs++;
        }
        if (flags.close_stream && flags.use_reset) {
            CloseStream(connection, stream, true);
            return TransportError::kNone;
        }

        auto send = std::make_unique<StreamSendContext>(std::move(bytes), msquic_stream, enqueued_at);
        auto* send_context = send.get();
        msquic_stream->outstanding_sends.fetch_add(1);
        const auto send_flags = flags.close_stream ? QUIC_SEND_FLAG_FIN : QUIC_SEND_FLAG_NONE;
        const auto status = api_->StreamSend(msquic_stream->handle, &send_context->buffer, 1, send_flags, send_context);
        if (QUIC_FAILED(status)) {
            msquic_stream->outstanding_sends.fetch_sub(1);
            return TransportError::kQueueFull;
        }
        {
            std::lock_guard lock(msquic_stream->metrics_mutex);
            if (send_context->buffer.Length > 0) {
                msquic_stream->metrics.tx_stream_objects++;
                msquic_stream->metrics.tx_stream_bytes += send_context->buffer.Length;
            }
            msquic_stream->metrics.tx_queue_size.AddValue(msquic_stream->outstanding_sends.load());
        }
        send.release();
        return TransportError::kNone;
    }

    TransportError MsQuicTransport::EnqueueDatagram(const std::shared_ptr<Connection>& connection,
                                                    std::shared_ptr<const std::vector<std::uint8_t>> bytes,
                                                    const std::uint8_t priority,
                                                    [[maybe_unused]] const std::uint32_t ttl_ms)
    {
        ...
        const auto enqueued_at = static_cast<std::uint64_t>(tick_service_->get().count());
        auto send = std::make_unique<DatagramSendContext>(std::move(bytes), msquic_connection, enqueued_at);
        auto* send_context = send.get();
        ...
        {
            std::lock_guard lock(msquic_connection->metrics_mutex);
            msquic_connection->datagram_metrics.enqueued_objs++;
            msquic_connection->datagram_metrics.tx_dgrams++;
            msquic_connection->datagram_metrics.tx_dgrams_bytes += send_context->buffer.Length;
        }
        send.release();
        return TransportError::kNone;
    }
                                      ...
                                      const bool use_reset)
    {
        auto msquic_connection = AsMsQuicConnection(connection);
        auto msquic_stream = AsMsQuicStream(stream);
        if (!msquic_connection || !msquic_stream || msquic_stream->GetConnectionID() != msquic_connection->GetID()) {
            return;
        }
        CloseStream(connection, msquic_stream->GetStreamId(), use_reset);
    }
    ...
    std::shared_ptr<Stream> MsQuicTransport::CreateDataStream(const std::shared_ptr<Connection>& connection,
                                                              const std::uint8_t priority)
    {
        return CreateStream(connection, false, priority);
    }

    std::shared_ptr<Stream> MsQuicTransport::CreateControlStream(const std::shared_ptr<Connection>& connection)
    {
        return CreateStream(connection, false, 0);
    }

    std::shared_ptr<Stream> MsQuicTransport::CreateRequestStream(const std::shared_ptr<Connection>& connection)
    {
        return CreateStream(connection, true, 0);
    }
    ...
    void MsQuicTransport::QueueCallback(std::function<void()> callback)
    {
        if (!callback_queue_.Push(std::move(callback))) {
            QUICR_LOGGER_ERROR(logger_, "MsQuic callback queue overflowed");
        }
    }
    ...
    void MsQuicTransport::CallbackLoop()
    {
        while (auto callback = callback_queue_.BlockPop()) {
            try {
                ...
                QUICR_LOGGER_ERROR(logger_, "MsQuic application callback failed: {}", exception.what());
            } catch (...) {
                QUICR_LOGGER_ERROR(logger_, "MsQuic application callback failed with an unknown exception");
            }
        }
    }
    ...
    void MsQuicTransport::MaintenanceLoop()
    {
        ...
        while (!stop_.load()) {
            ...
            for (const auto& connection : connections) {
                const auto sample = connection->TakeMetricsSample();
                QueueCallback(
                  [connection, sample_time, sample]() { connection->ReportMetricsSample(sample_time, sample); });
            }
