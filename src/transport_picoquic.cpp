    ...
    switch (cb_mode) {
        ...
        case picoquic_packet_loop_wake_up:
            ...
            if (transport->Status() == TransportStatus::kShuttingDown) {
                ...
                /*
                 * Each shard drains only its own instance. The transport reaches
                 * kShutdown once Shutdown() has seen every shard's thread stop, which is
                 * the real completion signal when there is more than one shard.
                 */
                           ...
                           const EnqueueFlags flags)
{
    ...
    if (flags.use_reliable) {
        ...
        ConnData cd{
            ...
            std::move(bytes), static_cast<uint64_t>(tick_service_->get().count()),
        };
...
{
    if (bytes_ctx == nullptr || conn_ctx->dgram_tx_data == nullptr) {
        return;
    }
    ...
    std::lock_guard _(*conn_ctx->dgram_tx_data);

    const auto [out_data, expired] = conn_ctx->dgram_tx_data->Front();
    if (out_data.has_value()) {
        const auto data_ctx_it = conn_ctx->active_data_contexts.find(out_data->get().data_ctx_id);
        if (data_ctx_it == conn_ctx->active_data_contexts.end()) {
            SPDLOG_LOGGER_DEBUG(logger,
...
{
    /*
     * The SO_REUSEPORT group must stay fixed for the process lifetime: Linux rehashes
     * the group whenever the set of listening sockets changes, which would break live
     * connections on every other shard. A shard that fails to start therefore fails the
     * whole transport; it is never retried or respawned.
     */
...
PicoQuicTransport::StartClient()
{
    ...
    // No connection yet, and clients only ever run one shard.
...
PicoQuicTransport::ClientLoop()
{
    ...
    // Clients always run exactly one shard: a single connection is a single 4-tuple.
    auto& shard = *shards_[0];

    shard.loop_params.local_port = 0;
    shard.loop_params.local_af = PF_UNSPEC;
    shard.loop_params.dest_if = 0;
    shard.loop_params.socket_buffer_size = tconfig_.socket_buffer_size;
#ifdef ESP_PLATFORM
    shard.loop_params.socket_buffer_size = 0x2048;
#endif
    shard.loop_params.do_not_use_gso = 0;
    shard.loop_params.extra_socket_required = 0;
    shard.loop_params.prefer_extra_socket = 0;
    shard.loop_params.simulate_eio = 0;
    shard.loop_params.send_length_max = 0;

    shard.thread_ctx =
      picoquic_start_network_thread(shard.quic_ctx, &shard.loop_params, PqLoopCb, &shard, &shard.loop_return_value);

    if (shard.quic_ctx == nullptr || shard.thread_ctx == nullptr) {
    ...
    if (shard.thread_ctx->return_code) {
        SPDLOG_LOGGER_ERROR(
          logger, "Could not start client quic network thread error: {}", shard.thread_ctx->return_code);
        return false;
    }
...
PicoQuicTransport::CreateStream(std::uint64_t conn_id, std::uint64_t data_ctx_id, uint8_t priority)
{
    ...
    RunPqFunction(*shard_idx, [this, conn_id = conn_id, data_ctx_id = data_ctx_id, state, priority]() {
        {
            std::lock_guard _(state->mtx);
            state->stream_id = CreateStreamInternal(conn_id, data_ctx_id, priority);
        }
        state->cv.notify_all();
...
PicoQuicTransport::EraseStreamState(ConnectionContext& conn_ctx, DataContext* data_ctx, const std::uint64_t stream_id)
{
    ...
    if (conn_ctx.transport_mode == TransportMode::kWebTransport) {
        conn_ctx.wt_stream_to_data_ctx.erase(stream_id);
    }
}
