    ...
    switch (cb_mode) {
        ...
        case picoquic_packet_loop_wake_up:
            ...
            if (transport->Status() == TransportStatus::kShuttingDown) {
                QUICR_LOGGER_INFO(transport->logger_, "picoquic is shutting down shard {0}", shard->index);
...
PicoQuicTransport::Server()
{
    for (auto& shard : shards_) {
        ...
        QUICR_LOGGER_DEBUG(logger_, "Starting picoquic network thread for shard {}", shard->index);
        ...
        if (shard->quic_ctx == NULL || shard->quic_network_thread_ctx == NULL) {
            QUICR_LOGGER_ERROR(logger_, "Failed to start picoquic network thread for shard {}", shard->index);
            SetStatus(TransportStatus::kShutdown);
            return;
        }
...
PicoQuicTransport::ClientLoop()
{
    ...
    if (shard.quic_network_thread_ctx->return_code) {
        QUICR_LOGGER_ERROR(
          logger_, "Could not start client quic network thread error: {}", shard.quic_network_thread_ctx->return_code);
        return false;
    }
...
PicoQuicTransport::Shutdown()
{
    ...
    for (auto& shard : shards_) {
        if (shard->quic_network_thread_ctx != NULL) {
            QUICR_LOGGER_INFO(logger_, "Closing transport picoquic thread for shard {0}", shard->index);
