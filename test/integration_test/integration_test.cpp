...
TEST_CASE("MsQuic - Connection" * doctest::test_suite("msquic-only"))
{
    quicr::SessionManager session_mgr;
    auto server =
      MakeTestServer(session_mgr, std::nullopt, std::nullopt, std::nullopt, std::nullopt, TransportBackend::kMsQuic);
...
TEST_CASE("MsQuic - Interoperates with picoquic" * doctest::test_suite("msquic-only"))
{
    const auto test_connection = [](const TransportBackend server_backend, const TransportBackend client_backend) {
        quicr::SessionManager session_mgr;
        auto server =
          MakeTestServer(session_mgr, std::nullopt, std::nullopt, std::nullopt, std::nullopt, server_backend);
        auto [session, callbacks] =
...
TEST_CASE("MsQuic - Object delivery" * doctest::test_suite("msquic-only"))
{
    ...
    quicr::SessionManager session_mgr;
    auto server = MakeTestServer(session_mgr, std::nullopt, 2, std::nullopt, std::nullopt, server_backend);
    ...
    auto test_subgroup_roll = [&](const std::string& protocol_scheme) {
        ...
        auto publish_object = [&](uint64_t group_id,
            ...
            ObjectHeaders headers = { .group_id = group_id,
                                      ...
                                      .immutable_extensions = std::nullopt };
            std::optional<messages::StreamHeaderProperties> stream_mode;
            stream_mode.emplace(true,
                                subgroup_id_mode.has_value() ? *subgroup_id_mode : messages::SubgroupIdType::kExplicit,
                                false,
                                false,
                                first_object);
        ...
        publish_object(0, 5, 5, true, messages::SubgroupIdType::kSetFromFirstObject);
        ...
        for (const auto& obj : received) {
            ...
            const bool is_first_subgroup_object = (obj.group_id == 0 && obj.subgroup_id == 0 && obj.object_id == 0) ||
                                                  ...
                                                  (obj.group_id == 0 && obj.subgroup_id == 5 && obj.object_id == 5) ||
