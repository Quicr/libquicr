...
TEST_CASE("Transport factory honours the configured backend")
{
    ...
    auto transport = quicr::Transport::MakeClientTransport(remote, config, tick_service, nullptr);
    ...
    CHECK_THROWS_WITH_AS(quicr::Transport::MakeClientTransport(remote, config, tick_service, nullptr),
                         "MsQuic transport supports raw QUIC only",
                         std::runtime_error);

    remote.proto = quicr::TransportProtocol::kQuic;
    transport = quicr::Transport::MakeClientTransport(remote, config, tick_service, nullptr);
...
TEST_CASE("MsQuic connection retains a stream while start callbacks are pending")
{
    ...
    auto stream = std::make_shared<quicr::MsQuicStream>(0, connection->GetID());
